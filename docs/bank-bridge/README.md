...
- `KAFKA_TRANSACTIONAL_ID` – transactional.id Kafka‑producer (по умолчанию `bank-bridge`);
...
- `BANK_BRIDGE_HOST` – адрес HTTP‑сервера (по умолчанию `0.0.0.0`);
- `BANK_BRIDGE_PORT` – порт HTTP‑сервера (по умолчанию `8080`);
...
- `BANK_BRIDGE_CORE_URL` – адрес Core API для загрузки пользователей;
